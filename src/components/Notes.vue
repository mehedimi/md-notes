<template>
    <div class="min-w-[20rem] w-80 px-4 flex flex-col border-r border-gray-200">
        ...
        <ul class="mt-5 overflow-y-auto h-full notes">
          <li>
            <router-link active-class="is-active" :to="{name: routeName, params: routeParams(note.id)}" class="mb-3 bg-stone-50 p-3 rounded note inline-block w-full" v-for="(note, index) in notes.data" :key="index">
                <span class="text-stone-400 font-light text-xs">{{ note.date }}</span>
